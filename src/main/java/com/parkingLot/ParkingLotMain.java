			...
			switch(option)
			{
				case 1:
					if(i==0) {
						...
						if(size >0)
							i++;
						System.out.println(parkingLot.createParkingLot(size));
					}	
					else {
						if(parkingLot.getParkedCars().size()==parkingLot.getSizeOfParkingArea())
							System.out.println("Sorry, parking lot is full");
						else {	
							System.out.println("Enter the car number");
							String carNumber=sc.next().trim().split(" ")[0];
							System.out.println("Enter the car color");
							String carColor=sc.next().trim().split(" ")[0];
